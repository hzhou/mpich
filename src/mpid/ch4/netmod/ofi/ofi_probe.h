...
#define MPIDI_OFI_PROBE_VNIS \
    do { \
        /* NOTE: hashing is based on target rank */ \
        vni_src = MPIDI_vci_get_src(comm, comm->rank, tag) % MPIDI_OFI_global.num_vnis; \
        vni_dst = MPIDI_vci_get_dst(comm, comm->rank, tag) % MPIDI_OFI_global.num_vnis; \
    } while (0)
